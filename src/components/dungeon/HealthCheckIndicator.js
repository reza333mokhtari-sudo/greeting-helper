      ...
      <div className="flex items-center gap-2">
        <Tooltip>
          <TooltipTrigger asChild>
            <button onClick={performCheck} disabled={loading} className="flex items-center gap-1.5 transition-opacity hover:opacity-80">
              {loading ? (<Loader2 className="h-3.5 w-3.5 animate-spin text-muted-foreground"/>) : health?.database ? (<CheckCircle2 className="h-3.5 w-3.5 text-green-500"/>) : (<XCircle className="h-3.5 w-3.5 text-red-500"/>)}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="flex items-center gap-1.5">
              {authStatus === 'loading' ? (<Loader2 className="h-3.5 w-3.5 animate-spin text-muted-foreground"/>) : authStatus === 'authenticated' ? (<ShieldCheck className="h-3.5 w-3.5 text-blue-500"/>) : (<ShieldAlert className="h-3.5 w-3.5 text-orange-500"/>)}
              <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/80">
                Auth
              </span>
            </div>
