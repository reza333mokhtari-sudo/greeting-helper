  ...
  return (
    <section className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-1">
        <Button
          size="sm"
          variant={p.mode === "brush" ? "secondary" : "ghost"}
          className="h-6 text-[10px]"
          onClick={() => p.onMode("brush")}
        >
          <Brush className="mr-1 h-3 w-3" /> Brush
        </Button>
        <Button
          size="sm"
          variant={p.mode === "select" ? "secondary" : "ghost"}
          className="h-6 text-[10px]"
      ...
      {p.mode === "brush" && (
        <label className="block space-y-1">
          ...
          <Slider
            ...
            max={320}
            step={4}
            onValueChange={([v]) => p.onBrush(v ?? p.brush)}
          />
        </label>
      )}
      ...
      <div className="space-y-1.5 rounded-lg border border-border/60 bg-card/40 p-2">
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {FOG_STYLES.map((s) => (
            ...
            >
              ...
              />
              <span className="block px-1 py-0.5 text-[9px] text-muted-foreground group-hover:text-foreground">
                {s.label}
              </span>
            </button>
          ))}
        ...
        <label className="block space-y-1">
          ...
          <Slider
            ...
            max={1}
            step={0.05}
            onValueChange={([v]) => p.onSettings({ fogSoftness: v ?? p.softness })}
          />
        </label>
        ...
        <label className="block space-y-1">
          ...
          <Slider
            ...
            max={1}
            step={0.02}
            onValueChange={([v]) => p.onSettings({ fogGmOpacity: v ?? p.gmOpacity })}
          />
        </label>
        ...
        <label className="block space-y-1">
          ...
          <Slider
            ...
            max={3}
            step={0.05}
            onValueChange={([v]) => p.onSettings({ fogScale: v ?? p.scale })}
          />
        </label>
      ...
      <p className="text-[10px] leading-relaxed text-muted-foreground">
        Fog is drawn with the selected 2D asset — translucent to you, solid in Player View and
        shared links.
      </p>
    </section>
  );
