        ...
        <button
          ...
          className={`px-2.5 py-1 text-[11px] transition-colors ${
            value === o.value
              ...
              : "bg-card/40 text-muted-foreground hover:bg-accent"
          }`}
        >
          {o.label}
        </button>
      ...
      <div className="flex flex-col gap-5 p-4 min-w-0">
        <section>
          ...
          <Row label="Style">
            <Select
              value={s.gridStyle}
              onValueChange={(v) => onChange({ gridStyle: v as GridStyle })}
            >
        ...
        <section>
          ...
          <Row label="Rough">
            <Switch
              checked={props.ngon.rough}
              onCheckedChange={(v) => props.onNgon({ rough: v })}
            />
          </Row>
          <Row label="Sides">
            <NumSlider
              value={props.ngon.sides}
              min={3}
              max={48}
              onChange={(v) => props.onNgon({ sides: v })}
            />
          </Row>
          <Row label="Draw to">
            <Segmented
        ...
        <section>
          ...
          <Row label="Hatch spacing">
            <NumSlider
              value={s.hatchDensity}
              min={3}
              max={20}
              onChange={(v) => onChange({ hatchDensity: v })}
            />
          </Row>
        ...
        <section>
          ...
          <Row label="Line of sight">
            <Select
              value={s.losMode}
              onValueChange={(v) => onChange({ losMode: v as Settings["losMode"] })}
            >
          ...
          <p className="mt-2 rounded-md bg-muted/50 p-2 text-[10px] leading-relaxed text-muted-foreground">
            Place light sources with the light tool (F). Room outlines and pillars block sight;
            doors block it when “Blocks light” is on.
          </p>
        </section>
      </div>
