    ...
    <section className="space-y-4">
      ...
      <ScrollArea className="h-64">
        <div className="space-y-3 pr-4">
          {WORKFLOWS.map(w => (
            <div key={w.id} className="group rounded-xl border border-border/50 bg-muted/20 p-3 transition-colors hover:border-primary/30">
              ...
              <ul className="space-y-1.5">
                {w.steps.map((s, i) => (
                  <li key={i} className="flex gap-2 text-[10px] leading-tight text-muted-foreground">
